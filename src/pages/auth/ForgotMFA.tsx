...
const ForgotMFA: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await authService.forgotMFA(email);

      if (result.success) {
        setMessage(
          result.message ||
            "If an account with that email exists, we have sent MFA reset instructions."
        );
        setMessageType("success");
      } else {
        setMessage(
          result.message || "Failed to send MFA reset email. Please try again."
        );
        ...
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <div className="w-16 h-16 mx-auto mb-4 bg-green-100 rounded-full flex items-center justify-center">
              <ShieldQuestion className="w-8 h-8 text-green-600" />
            </div>
            <h2 className="text-3xl font-bold text-gray-900 mb-2">
              Reset MFA
            </h2>
            <p className="text-gray-500">
              Lost access to your authenticator app? We'll help you reset it.
            </p>
          </div>
          ...
          <div className="mt-6 p-4 bg-amber-50 rounded-xl border border-amber-100">
            ...
            <ul className="text-xs text-amber-700 space-y-1">
              <li>1. You'll receive an email with MFA reset instructions</li>
              <li>2. Follow the link in the email to reset your MFA</li>
              <li>3. Log in again and set up a new authenticator</li>
            </ul>
          </div>
